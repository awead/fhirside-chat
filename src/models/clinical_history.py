...
class PatientClinicalHistory(BaseModel):
    ...
    key_conditions: List[str] = Field(
        default_factory=list, description="List of key/active medical conditions"
    )
    active_medications: List[str] = Field(
        default_factory=list, description="List of active medications"
    )
    recent_encounters: List[str] = Field(
        default_factory=list, description="Recent encounter summaries"
    )
    generated_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC),
        description="Timestamp when history was generated",
    )
